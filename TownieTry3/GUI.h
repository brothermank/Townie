...
struct GuiElement {
	...
	GuiElement(SDL_Renderer * Rend, LTexture Icon = LTexture(), Vector2I Pos = Vector2I(0, 0), GuiT Type = GUndefined) {
		icon = Icon;
		pos = Pos;
		type = Type;
		rend = Rend;
...
struct GButton : GuiElement {
	...
	GButton(SDL_Renderer * rend, LTexture Icon, Vector2I Pos = Vector2I(0, 0), Vector2I Size = Vector2I(10, 10)) : GuiElement(rend, Icon, Pos, Button_t) {
		size = Size;
		isPressed = false;
	}
	void DrawElement() {
...
struct GTextField : GuiElement {
	...
	GTextField(SDL_Renderer * rend, string Text = "Placeholder", Vector2I Pos = Vector2I(0, 0), int Size = 40, SDL_Color Color = SDL_Color()) : GuiElement(rend, LTexture(), Pos, Text_t) {
	...
	Vector2I getSize() {
		...
		return Vector2I((int)(proportion * size), size);
	}
	void OnClick() {
		active = true;
